	...
	if listname == "fuel" then
		if minetest.get_craft_result({method="fuel", width=1, items={stack}}).time ~= 0 then
			if inv:is_empty("src") then
				meta:set_string("infotext", "Campfire has no wood")
			end
...
local function furnace_node_timer(pos, elapsed)
	...
	if fuel_time < fuel_totaltime then
		...
		fuel_time = fuel_time + .75

		-- If there is a cookable item then check if it is ready yet
		if cookable then
			src_time = src_time + .75
	...
	if fuel_time <= fuel_totaltime and fuel_totaltime ~= 0 then
		...
		swap_node(pos, "campfire:campfire_active")
	...
	local infotext = "Campfire " .. active .. "(Item: " .. item_state .. "; Fuel: " .. fuel_state .. ")"
...
minetest.register_node("campfire:campfire", {
	...
	on_blast = function(pos)
		...
		drops[#drops+1] = "campfire:campfire"
